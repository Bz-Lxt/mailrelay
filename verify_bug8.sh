...
BODY_FILE="/tmp/gogo-verify-8-resp-$$"
...
# 先投递一封正常信封，让 WAL 里有积压记录，并证明入队链路本身是通的。
ok=$(curl -s -o /dev/null -w '%{http_code}' -X POST "$BASE/v1/enqueue" \
  -H 'Content-Type: application/json' \
  -d '{"from":"a@local","to":"b@local","subject":"seed","body":"y"}' || true)
if [[ "$ok" != "201" ]]; then
  echo "[EXPECT] POST /v1/enqueue with a well-formed envelope returns 201"
  echo "[ACTUAL] HTTP $ok"
  exit 2
fi

# 复现：调用压缩端口。正常应在毫秒级返回计数快照；异常时会挂住直到 curl 超时。
# 用 --max-time 兜住挂起，避免脚本被卡死；命中超时即视为复现。
code=$(curl -s -o "$BODY_FILE" -w '%{http_code}' --max-time 6 -X POST "$BASE/v1/compact" || true)

# 段3 对外部可观测量断言。
echo "[EXPECT] POST /v1/compact returns 200 with counters snapshot"
echo "[ACTUAL] HTTP $code body=$(cat "$BODY_FILE" 2>/dev/null | tr -d '\n')"

# 复现到 bug -> exit 1
# 未复现（已修复或 main 干净，main 上该端口 404） -> exit 0
# 环境失败 -> exit 2
case "$code" in
  200)
    exit 0
